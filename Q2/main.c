...
#ifdef USE_RAND_FOR
      for (int i_rand = 0 ; i_rand < sizeof(RANDOM_SEEDS) / sizeof(unsigned) - 1;i_rand++)
      {
          random_seed = RANDOM_SEEDS[i_rand];
#else
...
#else
    #ifdef G_CUT
        while (data.number_of_packets_processed < RUNLENGTH && (data.g < 0.4 || data.number_of_packets_processed < RUNLENGTH/2 ))
    #else
